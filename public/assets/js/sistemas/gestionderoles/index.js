...
async function loadUsers() {
    try {
        const res  = await fetch('/sistemas/gestionderoles/roles', {
...
async function loadRoles() {
    try {
        const res  = await fetch('/sistemas/gestionderoles/get-roles', {
            headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrf() },
        });
        if (!res.ok) throw new Error();
        const data = await res.json();
...
async function loadPermissions() {
    try {
        const res  = await fetch('/sistemas/gestionderoles/get-permissions', {
            headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrf() },
        });
        if (!res.ok) throw new Error();
        const data = await res.json();
...
async function searchEmployees(query) {
    try {
        const res = await fetch(`/sistemas/gestionderoles/search-employees?query=${encodeURIComponent(query)}`, {
            headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrf() },
        });
        if (!res.ok) throw new Error();
...
function renderUsers(filteredList) {
    ...
    tbody.innerHTML = slice.map(user => {
        ...
        const avatarHtml = user.employee_photo
            ? `<img src="/${user.employee_photo}" alt="${user.name}">`
...
/* ── VIEW USER ── completamente rediseñado ── */
...
async function deleteUser(userId) {
    ...
    try {
        const res  = await fetch(`/sistemas/gestionderoles/roles/${userId}`, {
            method: 'DELETE',
            headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': csrf() },
        });
        const data = await res.json();
...
if (typeof $ !== 'undefined') {
    $(document).ready(function () {
        ...
        $('#permissionsForm').on('submit', function (e) {
            ...
            const url    = editingUserId ? `/sistemas/gestionderoles/roles/${editingUserId}` : '/sistemas/gestionderoles/roles';
