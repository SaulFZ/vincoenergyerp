...
@section('content')
    <style>
        ...
        /* ─── ESTADÍSTICAS ────────────────────────────────────────────────── */
        .stats-row {
            display: grid; grid-template-columns: repeat(5, 1fr); gap: 14px; padding: 4px 0 16px;
        }

        .stat-card {
            background: #fff; border: 1px solid #e2e8f0; border-radius: 16px;
            padding: 18px 16px; display: flex; justify-content: space-between;
            align-items: center; gap: 10px; min-height: 90px;
            box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02); transition: transform .3s ease, box-shadow .3s ease;
            position: relative; overflow: hidden;
        }

        .stat-card::before {
            content: ''; position: absolute; bottom: 0; left: 0; right: 0;
            height: 4px; border-radius: 0 0 16px 16px; transition: opacity .3s; opacity: 0.85;
        }

        .stat-card:hover { transform: translateY(-4px); box-shadow: 0 12px 24px -8px rgba(0, 0, 0, 0.08); }
        .stat-card:hover::before { opacity: 1; }

        .stat-card.c-main { background: linear-gradient(135deg, var(--color-primary) 0%, #1a2a33 100%); border: none; }
        .stat-card.c-main::before { display: none; }
        .stat-card.c-wait::before { background: var(--s-wait-border); }
        .stat-card.c-pend::before { background: var(--s-pend-border); }
        .stat-card.c-done::before { background: var(--s-done-border); }
        .stat-card.c-cancel::before { background: var(--s-cancel-border); }

        .stat-info { display: flex; flex-direction: column; gap: 4px; }
        .stat-label { font-size: .65rem; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: #64748b; }
        .c-main .stat-label { color: rgba(255, 255, 255, .7); }
        .stat-value { font-size: 2rem; font-weight: 700; color: #0f172a; line-height: 1; font-family: 'IBM Plex Mono', monospace; }
        .c-main .stat-value { color: #fff; }
        .stat-sub { font-size: .7rem; color: #94a3b8; font-weight: 500; }
        .c-main .stat-sub { color: rgba(255, 255, 255, .5); }

        .stat-icon {
            width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center;
            justify-content: center; font-size: 1.25rem; flex-shrink: 0;
        }
        .c-main .stat-icon { background: rgba(255, 255, 255, .1); color: #fff; }
        .c-wait .stat-icon { background: var(--s-wait-bg); color: var(--s-wait-text); }
        .c-pend .stat-icon { background: var(--s-pend-bg); color: var(--s-pend-text); }
        .c-done .stat-icon { background: var(--s-done-bg); color: var(--s-done-text); }
        .c-cancel .stat-icon { background: var(--s-cancel-bg); color: var(--s-cancel-text); }
        ...
        .search-wrap { flex-grow: 1; position: relative; min-width: 220px; }
        .search-wrap i { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: #94a3b8; font-size: .85rem; }
        .search-wrap input { width: 100%; padding: 10px 14px 10px 38px; border: 1px solid #cbd5e1; border-radius: 8px; background: #f8fafc; font-size: .85rem; transition: all .2s; }
        .search-wrap input:focus { border-color: var(--color-primary); background: #fff; outline: none; box-shadow: 0 0 0 3px rgba(52, 73, 85, .1); }
        select.f-sel { border: 1px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; outline: none; background: #f8fafc; color: #1e293b; font-size: .85rem; font-weight: 500; transition: all .2s; cursor: pointer; }
        select.f-sel:focus, select.f-sel:hover { border-color: var(--color-primary); background: #fff; }

        .main-table { width: 100%; border-collapse: separate; border-spacing: 0; }
        .main-table th { background: #f1f5f9; color: #475569; padding: 12px 16px; text-align: left; font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; border-bottom: 2px solid #e2e8f0; }
        .main-table th:first-child { border-top-left-radius: 10px; }
        .main-table th:last-child { border-top-right-radius: 10px; }
        .main-table td { padding: 12px 16px; border-bottom: 1px solid #f1f5f9; color: #334155; font-size: .85rem; vertical-align: middle; transition: background .2s; }
        .main-table tr:hover td { background: #f8fafc; }
        .main-table tr:last-child td { border-bottom: none; }

        .tk-num, .tk-folio { font-family: 'IBM Plex Mono', monospace; font-size: .85rem; }
        .tk-num { font-weight: 600; color: #0f172a; }
        .tk-folio { font-weight: 700; color: var(--color-primary); }
        ...
        /* ─── STATUS TAGS ─────────────────────────────────────────────────── */
        .stag { display: inline-flex; align-items: center; gap: 6px; padding: 5px 10px; border-radius: 6px; font-size: .68rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; white-space: nowrap; }
        ...
        .ptag { display: inline-flex; align-items: center; gap: 4px; padding: 4px 8px; border-radius: 5px; font-size: .68rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; }
        ...
        .ptag.sin-clasificar { background: #f8fafc; color: #64748b; border: 1px solid #cbd5e1; }

        /* ─── CAMPOS DE FORMULARIO COMUNES ────────────────────────────────── */
        .form-group { margin-bottom: 14px; }
        .form-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        .form-divider { height: 1px; background: #e2e8f0; margin: 16px 0; border: none; }
        .form-group label { display: flex; align-items: center; gap: 6px; font-size: .7rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; color: #475569; margin-bottom: 8px; }

        .field-wrap { position: relative; display: flex; align-items: center; transition: all .2s; }
        .field-icon { position: absolute; left: 14px; font-size: .8rem; pointer-events: none; color: #94a3b8; transition: color .2s; }

        .ctrl { width: 100%; padding: 10px 14px 10px 36px; background: #fff; border: 1px solid #cbd5e1; border-radius: 8px; font-size: .85rem; color: #1e293b; transition: all .2s; }
        .ctrl.no-icon { padding-left: 14px; }
        .ctrl:focus { outline: none; border-color: var(--color-primary); box-shadow: 0 0 0 3px rgba(52, 73, 85, .1); }
        textarea.ctrl { resize: none; overflow: hidden; min-height: 42px; line-height: 1.5; }
        ...
        .ctrl.bg-s-nuevo { background: var(--s-new-bg) !important; color: var(--s-new-text) !important; border-color: var(--s-new-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-new-text) !important; opacity: 1 !important; }
        .ctrl.bg-s-abierto { background: var(--s-open-bg) !important; color: var(--s-open-text) !important; border-color: var(--s-open-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-open-text) !important; opacity: 1 !important; }
        .ctrl.bg-s-en-espera { background: var(--s-wait-bg) !important; color: var(--s-wait-text) !important; border-color: var(--s-wait-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-wait-text) !important; opacity: 1 !important; }
        .ctrl.bg-s-por-concluir { background: var(--s-pend-bg) !important; color: var(--s-pend-text) !important; border-color: var(--s-pend-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-pend-text) !important; opacity: 1 !important; }
        .ctrl.bg-s-realizado { background: var(--s-done-bg) !important; color: var(--s-done-text) !important; border-color: var(--s-done-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-done-text) !important; opacity: 1 !important; }
        .ctrl.bg-s-cancelado { background: var(--s-cancel-bg) !important; color: var(--s-cancel-text) !important; border-color: var(--s-cancel-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-cancel-text) !important; opacity: 1 !important; }

        .ctrl.bg-p-alta { background: var(--s-cancel-bg) !important; color: var(--s-cancel-text) !important; border-color: var(--s-cancel-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-cancel-text) !important; opacity: 1 !important; }
        .ctrl.bg-p-media { background: var(--s-pend-bg) !important; color: var(--s-pend-text) !important; border-color: var(--s-pend-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-pend-text) !important; opacity: 1 !important; }
        .ctrl.bg-p-baja { background: var(--s-done-bg) !important; color: var(--s-done-text) !important; border-color: var(--s-done-border) !important; font-weight: 700; -webkit-text-fill-color: var(--s-done-text) !important; opacity: 1 !important; }
        .ctrl.bg-p-sin-clasificar { background: #f8fafc !important; color: #64748b !important; border-color: #cbd5e1 !important; font-weight: 700; -webkit-text-fill-color: #64748b !important; opacity: 1 !important; }

        /* Comportamiento base para deshabilitados */
        .ctrl[readonly], .ctrl:disabled:not([class*="bg-"]) { background: #f8fafc; cursor: default; border-color: #e4eef5; color: #475569; opacity: 1; -webkit-text-fill-color: #475569; }

        /* ─── MODALES FLOTANTES REDISEÑADOS ───────────────────────────────── */
        .modal-overlay {
            position: fixed; inset: 0; background: rgba(15, 23, 42, .75); backdrop-filter: blur(5px);
            z-index: 2000; display: flex; align-items: center; justify-content: center;
            opacity: 0; visibility: hidden; transition: all .3s cubic-bezier(0.4, 0, 0.2, 1);
            padding: 20px;
        }
        .modal-overlay.open { opacity: 1; visibility: visible; }

        .modal-box {
            background: #fff; border-radius: 18px; width: 100%; max-width: 580px; max-height: 92vh;
            display: flex; flex-direction: column; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.35);
            transform: scale(.95) translateY(20px); transition: transform .4s cubic-bezier(.16, 1, .3, 1);
            position: relative; overflow: hidden; border: 1px solid rgba(255,255,255,0.1);
        }
        .modal-overlay.open .modal-box { transform: scale(1) translateY(0); }

        #modal-ticket .modal-box { max-width: 950px; }
        ...
        .modal-head h3 { margin: 0; font-size: 1.15rem; font-weight: 700; display: flex; align-items: center; gap: 10px; }
        .modal-head .sub { font-size: .8rem; font-weight: 500; color: #cbd5e1; margin-top: 6px; display: flex; gap: 14px; align-items: center;}

        .modal-body { padding: 26px; overflow-y: auto; background: #fff; flex-grow: 1; position: relative; }
        .modal-body::-webkit-scrollbar { width: 8px; }
        .modal-body::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; border: 2px solid #fff; }
        .modal-body::-webkit-scrollbar-thumb:hover { background: #94a3b8; }

        .modal-foot {
            background: #f8fafc; padding: 18px 26px; border-top: 1px solid #e2e8f0;
        ...
        .modal-overlay.mode-view .modal-foot { display: none !important; }
        .modal-overlay.mode-view .modal-body { border-radius: 0 0 18px 18px; padding-bottom: 30px; }
        ...
        .modal-overlay.mode-view .ts-left .field-icon:not(.bg-icon) { color: #94a3b8; }
        /* Mantenemos los colores dinámicos de los selectores, pero les quitamos la flecha nativa para que se vean fijos */
        .modal-overlay.mode-view .ts-left select.ctrl { appearance: none; -webkit-appearance: none; }

        /* ─── GRID INTERNO DEL MODAL DE TICKET ────────────────────────────── */
        .ticket-split-view { display: grid; grid-template-columns: 1fr 360px; gap: 28px; align-items: start; }
        .ts-left { display: flex; flex-direction: column; gap: 4px; }
        .ts-right {
            background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 14px;
            padding: 20px; max-height: 550px; overflow-y: auto; box-shadow: inset 0 2px 4px rgba(0,0,0,0.01);
        }
        @media (max-width: 850px) { .ticket-split-view { grid-template-columns: 1fr; } .ts-right { max-height: none; } }

        /* ─── META STRIP EN MODAL ─────────────────────────────────────────── */
        .meta-strip {
            display: grid; grid-template-columns: repeat(3, 1fr); gap: 1px; background: #e2e8f0;
            border: 1px solid #cbd5e1; border-radius: 10px; overflow: hidden; margin-bottom: 24px;
        }
        .meta-cell { background: #fff; padding: 12px 16px; display: flex; flex-direction: column; gap: 4px; }
        .meta-lbl { font-size: .65rem; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; color: #64748b; display: flex; align-items: center; gap: 6px; }
        .meta-val { font-size: .9rem; font-weight: 700; color: #0f172a; display: flex; align-items: center; gap: 6px; }

        /* ─── BOTONERA DE ESTADOS (FOOTER) ────────────────────────────────── */
        .status-actions { display: flex; flex-direction: column; gap: 10px; flex-grow: 1; }
        .status-bar-label { font-size: .68rem; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: .05em; }
        .status-bar { display: flex; flex-wrap: wrap; gap: 8px; }
        .status-btn {
            flex: 1; min-width: 85px; padding: 10px 12px; font-size: .72rem; font-weight: 700;
            text-transform: uppercase; border-radius: 8px; border: 1px solid #cbd5e1;
            background: #fff; color: #64748b; cursor: pointer; transition: all .2s; text-align: center;
        }
        .status-btn:hover { background: #f1f5f9; color: #334155; border-color: #94a3b8; }
        .status-btn.on[data-v="en-espera"] { background: var(--s-wait-bg); color: var(--s-wait-text); border-color: var(--s-wait-border); box-shadow: 0 3px 6px rgba(193, 90, 0, .15); }
        .status-btn.on[data-v="por-concluir"] { background: var(--s-pend-bg); color: var(--s-pend-text); border-color: var(--s-pend-border); box-shadow: 0 3px 6px rgba(196, 125, 0, .15); }
        .status-btn.on[data-v="realizado"] { background: var(--s-done-bg); color: var(--s-done-text); border-color: var(--s-done-border); box-shadow: 0 3px 6px rgba(26, 140, 56, .15); }
        .status-btn.on[data-v="cancelado"] { background: var(--s-cancel-bg); color: var(--s-cancel-text); border-color: var(--s-cancel-border); box-shadow: 0 3px 6px rgba(185, 28, 28, .15); }

        /* ─── BOTONES GENERALES ───────────────────────────────────────────── */
        .btn { padding: 10px 20px; border-radius: 8px; cursor: pointer; border: none; font-weight: 600; font-size: .85rem; display: inline-flex; align-items: center; gap: 8px; transition: all .2s; justify-content: center; }
        .btn-primary { background: var(--color-primary); color: #fff; box-shadow: 0 2px 4px rgba(52, 73, 85, .15); }
        .btn-primary:hover { background: var(--color-medium); transform: translateY(-1px); box-shadow: 0 4px 12px rgba(52, 73, 85, .25); }
        ...
        .tbl-actions { display: flex; gap: 8px; justify-content: center; }
        .tbl-actions button { width: 32px; height: 32px; border-radius: 8px; border: 1px solid transparent; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: .2s; font-size: .85rem; }
        .btn-view { background: #e0f2fe; color: #0284c7; border-color: #bae6fd; }
        .btn-attend { background: #ffedd5; color: #ea580c; border-color: #fdba74; }
        .btn-view:hover, .btn-attend:hover { transform: translateY(-2px); box-shadow: 0 4px 8px rgba(0, 0, 0, .08); }

        .btn-close-modal { background: rgba(255, 255, 255, .1); border: 1px solid rgba(255, 255, 255, .2); color: #fff; width: 32px; height: 32px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all .25s; flex-shrink: 0; font-size: .9rem; }
        .btn-close-modal:hover { background: #ef4444; border-color: #ef4444; transform: rotate(90deg); }

        /* ─── BLOQUE DE COMENTARIO ────────────────────────────────────────── */
        .comment-block { background: #fffbeb; border: 1px solid var(--s-pend-border); border-radius: 10px; padding: 14px 16px; margin-top: 10px; display: none; }
        .comment-block.show { display: block; animation: slideDown .3s ease; }
        .comment-block.danger { background: #fef2f2; border-color: var(--s-cancel-border); }
        .comment-block.danger label { color: var(--s-cancel-text); }
        .comment-block.danger .ctrl { border-color: #fca5a5; background: #fff; }
        @keyframes slideDown { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }

        /* ─── TIMELINE ────────────────────────────────────────────────────── */
        .timeline-title { font-size: .75rem; font-weight: 700; color: #475569; text-transform: uppercase; margin-bottom: 20px; display: flex; align-items: center; gap: 8px; padding-bottom: 12px; border-bottom: 1px solid #cbd5e1; letter-spacing: .05em; }
        .tl-item { margin-bottom: 18px; padding-left: 20px; border-left: 2px solid var(--color-primary); position: relative; }
        .tl-item::before { content: ''; position: absolute; left: -6px; top: 3px; width: 10px; height: 10px; border-radius: 50%; background: var(--color-primary); border: 2px solid #f8fafc; box-shadow: 0 0 0 2px #cbd5e1; }
        .tl-date { font-size: .65rem; color: #64748b; font-weight: 700; margin-bottom: 4px; font-family: 'IBM Plex Mono', monospace; }
        .tl-user { font-size: .8rem; font-weight: 700; color: #0f172a; margin-bottom: 6px; display: flex; align-items: center; gap: 8px; }
        .tl-badge { font-size: .6rem; padding: 3px 6px; border-radius: 4px; background: #e2e8f0; color: #475569; text-transform: uppercase; letter-spacing: .03em;}
        .tl-msg { font-size: .8rem; color: #334155; background: #fff; padding: 12px 14px; border-radius: 8px; border: 1px solid #e2e8f0; line-height: 1.5; box-shadow: 0 1px 2px rgba(0,0,0,0.02); }
        .tl-empty { text-align: center; padding: 40px 10px; color: #94a3b8; font-size: .85rem; font-weight: 500; display: flex; flex-direction: column; align-items: center; gap: 10px; }
        .tl-empty i { font-size: 1.8rem; color: #cbd5e1; }

        /* ─── LOADER ──────────────────────────────────────────────────────── */
        .modal-loader { position: absolute; inset: 0; background: rgba(255, 255, 255, .9); backdrop-filter: blur(3px); display: flex; flex-direction: column; align-items: center; justify-content: center; z-index: 50; border-radius: 0 0 18px 18px; transition: opacity .3s; }
        .loader-spin { width: 44px; height: 44px; border: 3px solid #e2e8f0; border-top-color: var(--color-primary); border-radius: 50%; animation: spin .8s cubic-bezier(.4, 0, .2, 1) infinite; margin-bottom: 16px; }
        .modal-loader p { font-size: .85rem; font-weight: 600; color: #475569; letter-spacing: .05em; text-transform: uppercase; }
        @keyframes spin { to { transform: rotate(360deg); } }

        /* ─── PAGINACIÓN ──────────────────────────────────────────────────── */
        .pager { display: flex; justify-content: space-between; align-items: center; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e2e8f0; }
        .pager-info { font-size: .85rem; color: #64748b; font-weight: 600; }
        .pager-btns { display: flex; gap: 6px; }
        .pager-btn { background: #fff; border: 1px solid #cbd5e1; color: #334155; width: 34px; height: 34px; border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: .85rem; font-weight: 600; transition: all .2s; }
        .pager-btn:hover:not(:disabled) { background: #f8fafc; border-color: var(--color-primary); color: var(--color-primary); }
        .pager-btn.on { background: var(--color-primary); color: #fff; border-color: var(--color-primary); box-shadow: 0 2px 4px rgba(52, 73, 85, .2); }
                ...
                <form id="form-new">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <div class="field-wrap">
                                ...
                                <input type="text" class="ctrl" value="{{ auth()->user()->employee->area->name ?? 'Área No Asignada' }}" disabled>
                                <input type="hidden" id="n-area" value="{{ auth()->user()->employee->area->code ?? 'SIS' }}">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="field-wrap">
                                ...
                                <input type="text" class="ctrl" value="{{ auth()->user()->name ?? 'Usuario Actual' }}" disabled>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="field-wrap">
                            ...
                            <input type="text" id="n-asunto" class="ctrl" placeholder="Ej: Falla en el acceso al servidor..." required>
                        </div>
                    </div>

                    <div class="form-group" style="margin-bottom: 0;">
                        <label><i class="fas fa-align-left"></i> Descripción Detallada *</label>
                        <textarea id="n-desc" class="ctrl no-icon" rows="3" placeholder="Proporcione los detalles de la falla o solicitud..." required></textarea>
                    </div>
                </form>
            </div>
        ...
        <div class="modal-box">
            ...
            <form id="ticket-form" style="display: flex; flex-direction: column; flex-grow: 1; overflow: hidden;">
                ...
                <div class="modal-foot" id="p-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="btn-save" style="margin-left: 20px; padding: 12px 24px;">
...
@push('scripts')
    <script>
        ...
        const STATUS_COLORS = {
            nuevo: 'var(--s-new-text)', abierto: 'var(--s-open-text)', 'en-espera': 'var(--s-wait-text)',
            'por-concluir': 'var(--s-pend-text)', realizado: 'var(--s-done-text)', cancelado: 'var(--s-cancel-text)',
        };
        const PRIO_COLORS = {
            alta: 'var(--s-cancel-text)', media: 'var(--s-pend-text)', baja: 'var(--s-done-text)', 'sin-clasificar': '#64748b',
        };
        ...
        function renderTable() {
            ...
            if (!filteredTickets.length) {
                tbody.innerHTML = `<tr><td colspan="9" style="text-align:center;padding:20px;color:#64748b;">No hay tickets que coincidan.</td></tr>`;
        ...
        document.getElementById('form-new').addEventListener('submit', async e => {
            ...
            try {
                ...
                if (r.ok && res.success) {
                    Swal.fire({
                            title: '¡Ticket Generado!', text: `Folio asignado: ${res.folio}.`,
                            icon: 'success', confirmButtonColor: '#1e293b'
                        }).then(() => { closeNewModal(); loadTickets(); });
            ...
            try {
                ...
                if (action === 'atender' && d.status === 'nuevo') {
                    const upd = await fetch(`{{ url('/systems/tickets/update-status') }}/${id}`, {
                        ...
                        body: JSON.stringify({
                            status: 'abierto', priority: d.priority, subject: d.subject,
                            description: d.description, comentario: 'El ticket ha sido tomado para su atención.'
                        })
                    });
                    if (upd.ok) {
                        d.status = 'abierto';
                        ...
                        d.trackings.push({
                            created_at: new Date().toISOString(), user: { name: '{{ auth()->user()->name ?? 'Sistema' }}' },
                            status_after: 'abierto', message: 'El ticket ha sido tomado para su atención.'
                        });
                    }
                }
                ...
                if (action === 'atender') {
                    ...
                    if (d.status === 'por-concluir') {
                        btns.forEach(b => { if (!['realizado', 'cancelado'].includes(b.dataset.v)) b.style.display = 'none'; });
                    }
        ...
        document.getElementById('ticket-form').addEventListener('submit', async e => {
            ...
            if (prioridad === 'sin clasificar') {
                Swal.fire({ title: 'Prioridad Requerida', text: 'Debes asignar una prioridad (Alta, Media o Baja).', icon: 'warning', confirmButtonColor: '#1e293b' });
                return;
            }
            if (['por-concluir', 'cancelado'].includes(estado) && !comentario.trim()) {
                Swal.fire({ title: 'Justificación obligatoria', text: 'Debe agregar un comentario.', icon: 'warning', confirmButtonColor: '#1e293b' });
                return;
            }

            const payload = {
                status: estado, priority: prioridad, subject: document.getElementById('p-asunto').value,
                description: document.getElementById('p-desc').value, comentario: comentario,
            };
            ...
            try {
                ...
                const res = await r.json();
                if (r.ok && res.success) {
                    Swal.fire({ title: '¡Gestión guardada!', text: res.message, icon: 'success', confirmButtonColor: '#1e293b' })
                        .then(() => { closeTicketModal(); loadTickets(); });
                } else throw new Error(res.message);
            } catch (err) {
                Swal.fire('Error', err.message, 'error');
            } finally {
