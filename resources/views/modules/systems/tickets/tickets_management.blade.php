...
@section('content')
    <style>
        ...
        .tickets-layout.panel-off {
            grid-template-columns: 0 1fr;
            gap: 0;
        }

        /* ─── ESTADÍSTICAS (DISEÑO ACTUALIZADO) ───────────────────────────── */
        .stats-row {
            display: grid;
            grid-template-columns: repeat(5, 1fr); /* Mantiene el ancho original */
            gap: 14px;
        ...
        .stat-card {
            ...
            align-items: center;
            gap: 10px;
            min-height: 90px; /* Mantiene la altura original */
        ...
        /* Línea inferior de color estilo analítico */
        .stat-card::before {
            content: '';
            position: absolute;
        ...
        .stat-card.c-main {
            background: linear-gradient(140deg, var(--color-primary) 0%, #263840 100%);
            border: none;
        }
        ...
        .stat-value {
            ...
            color: #0c1e2e;
            line-height: 1;
            font-family: 'IBM Plex Mono', monospace; /* Usando fuente mono para números */
        }
        ...
        .c-main .stat-sub { color: rgba(255, 255, 255, .55); }
        ...
        /* Usando las paletas de color nativas, eliminando el borde para un look más limpio */
        ...
        .field-icon {
            position: absolute; left: 12px; font-size: .75rem; pointer-events: none;
            color: #8da4b7; transition: color .2s;
        }

        .ctrl {
            width: 100%; padding: 8px 12px 8px 30px; background: #fff;
            border: 1px solid #cbd5e1; border-radius: 7px; font-size: .8rem;
        ...
        .ctrl:focus {
            ...
            box-shadow: 0 0 0 3px rgba(52, 73, 85, .12);
        }

        textarea.ctrl { resize: none; overflow: hidden; min-height: 38px; line-height: 1.4; }

        .ctrl[readonly], .ctrl:disabled {
            background: #f8fafc; cursor: default; border-color: #e4eef5;
            color: #475569; opacity: 1; -webkit-text-fill-color: #475569;
        }
        ...
        .ctrl.bg-p-sin-clasificar { background: #f1f5f9 !important; color: #475569 !important; border-color: #cbd5e1 !important; font-weight: 700; -webkit-text-fill-color: #475569 !important; }

        /* ─── BLOQUE DE COMENTARIO ────────────────────────────────────────── */
        .comment-block {
            background: #fff9ed; border: 1px solid var(--s-pend-border);
            border-radius: 8px; padding: 10px 12px; margin-bottom: 12px; display: none;
        }
        ...
        /* ─── BOTONERA DE ESTADOS ─────────────────────────────────────────── */
        .status-bar-label {
            font-size: .65rem; font-weight: 700; color: #5c7a96; text-transform: uppercase;
        ...
        .status-btn {
            flex: 1; min-width: 70px; padding: 6px; font-size: .65rem; font-weight: 700;
            text-transform: uppercase; border-radius: 6px; border: 1px solid #cbd5e1;
        ...
        .tl-item::before {
            ...
            border-radius: 50%; background: var(--color-primary); border: 2px solid #f8fafc;
        }
        ...
        /* ─── TABLA ───────────────────────────────────────────────────────── */
        .data-section {
            background: #fff; padding: 20px; border-radius: 14px; border: 1px solid #c4d4e3;
            box-shadow: 0 4px 12px rgba(0, 0, 0, .03); min-width: 0;
        }
        ...
        .tk-num, .tk-folio { font-family: 'IBM Plex Mono', monospace; font-size: .85rem; color: #000; }
        .tk-num { font-weight: 600; } .tk-folio { font-weight: 700; }
        ...
        .ptag.sin-clasificar { background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; }
        ...
        .btn-close-panel { background: rgba(255, 255, 255, .12); border: 1px solid rgba(255, 255, 255, .25); color: #fff; width: 28px; height: 28px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: .25s; flex-shrink: 0; font-size: .85rem; }
        ...
        /* ─── MODAL FLOTANTE ──────────────────────────────────────────────── */
        .modal-overlay { position: fixed; inset: 0; background: rgba(10, 22, 40, .72); backdrop-filter: blur(5px); z-index: 2000; display: flex; align-items: center; justify-content: center; opacity: 0; visibility: hidden; transition: all .3s ease; }
        .modal-overlay.open { opacity: 1; visibility: visible; }
        .modal-box { background: #fff; border-radius: 20px; width: 100%; max-width: 620px; max-height: 90vh; display: flex; flex-direction: column; box-shadow: 0 30px 60px -10px rgba(0, 0, 0, .35); border: 1px solid #b8ccdc; transform: scale(.93) translateY(28px); transition: transform .4s cubic-bezier(.16, 1, .3, 1); }
        .modal-overlay.open .modal-box { transform: scale(1) translateY(0); }
        .modal-head { display: flex; justify-content: space-between; align-items: center; padding: 16px 22px; background: linear-gradient(135deg, var(--color-primary) 0%, #263840 100%); color: #fff; border-radius: 19px 19px 0 0; flex-shrink: 0; }
        ...
        <div class="modal-box">
            ...
            <div class="modal-body">
                ...
                <form id="form-new">
                    ...
                    <div class="form-group">
                        <label><i class="fas fa-align-left"></i> Descripción Detallada *</label>
                        <textarea id="n-desc" class="ctrl no-icon" rows="1" placeholder="Proporcione los detalles de la falla o solicitud..." required></textarea>
                    </div>
        ...
        document.getElementById('panel-form').addEventListener('submit', async e => {
            ...
            if (prioridad === 'sin clasificar') {
                Swal.fire({
                    ...
                    text: 'Debes asignarle una prioridad real (Alta, Media o Baja) al ticket antes de guardar.',
                    icon: 'warning',
                    confirmButtonColor: '#344955'
                });
                return;
            ...
            if (['por-concluir', 'cancelado'].includes(estado) && !comentario.trim()) {
                Swal.fire({
                    ...
                    text: 'Debe agregar un comentario para este estatus.',
                    icon: 'warning',
                    confirmButtonColor: '#344955'
                });
                return;
