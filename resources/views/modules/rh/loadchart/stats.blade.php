...
<style>
    ...
    /* ⭐ ESTILOS DE COMISIONES MODIFICADOS PARA HACERLOS CLICKEABLES */
    .has-commissioned {
    ...
    .commissioned-icon {
        ...
        margin-right: 4px;
        font-size: 14px;
        pointer-events: none; /* Para que el clic vaya a la celda entera */
    }

    /* ⭐ MODAL CUSTOM PARA DETALLE DE COMISIONES */
    ...
    .custom-modal-dialog {
        background: var(--white); border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.2);
    ...
    .custom-modal-header {
        ...
        padding: 15px 20px; border-bottom: 1px solid var(--border-color); background: #f8fafc; border-radius: 12px 12px 0 0;
    }
    .custom-modal-header h5 { margin: 0; font-size: 1.1rem; font-weight: 700; color: var(--secondary-color); }
    .custom-modal-header .close-btn { background: transparent; border: none; font-size: 1.5rem; cursor: pointer; color: var(--text-medium); }
    .custom-modal-body { padding: 20px; }
    .custom-modal-footer { padding: 15px 20px; border-top: 1px solid var(--border-color); display: flex; justify-content: flex-end; }
...
{{-- ⭐ MODAL HTML PARA DETALLES DE COMISIONADOS --}}
<div id="commissionedModal" class="custom-modal-backdrop">
    <div class="custom-modal-dialog">
        <div class="custom-modal-content">
            <div class="custom-modal-header">
                <h5><i class="fas fa-exchange-alt"></i> Desglose de Comisiones Recibidas</h5>
                <button type="button" class="close-btn" onclick="closeCommissionedModal()">&times;</button>
            </div>
            <div class="custom-modal-body">
                ...
                <div style="display: flex; gap: 15px; margin-bottom: 20px; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 150px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
                        <span style="font-size: 11px; color: #64748b; font-weight: bold; text-transform: uppercase;">Monto del Área (Normal)</span><br>
                        <strong style="font-size: 18px; color: #334c95;" id="comm-modal-normal">$0</strong>
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #f0f9ff; padding: 15px; border-radius: 8px; border: 1px solid #bae6fd;">
                        <span style="font-size: 11px; color: #0ea5e9; font-weight: bold; text-transform: uppercase;">Monto Comisionado (Extras)</span><br>
                        <strong style="font-size: 18px; color: #0369a1;" id="comm-modal-comisionado">$0</strong>
                    </div>
                    <div style="flex: 1; min-width: 150px; background: #f0fdf4; padding: 15px; border-radius: 8px; border: 1px solid #bbf7d0;">
                        ...
                        <strong style="font-size: 18px; color: #15803d;" id="comm-modal-total">$0</strong>
                    </div>
                </div>

                <h6 style="margin-bottom: 10px; font-weight: bold; color: #4a5568;">Historial Detallado de Comisiones</h6>
                <div style="border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden;">
                    <table class="data-table" style="width: 100%; font-size: 13px; margin: 0;">
                        <thead>
                            <tr>
                                <th class="text-left" style="background: #e2e8f0; color: #4a5568; border-right: none;">Fecha</th>
                                <th class="text-left" style="background: #e2e8f0; color: #4a5568; border-right: none;">Empleado</th>
                                <th class="text-left" style="background: #e2e8f0; color: #4a5568; border-right: none;">Área Origen</th>
                                <th class="text-right" style="background: #e2e8f0; color: #4a5568; border-right: none;">Monto</th>
                            </tr>
                        </thead>
                        <tbody id="comm-modal-tbody">
                            </tbody>
                    </table>
                </div>
            </div>
            <div class="custom-modal-footer">
                <button class="btn btn-secondary" onclick="closeCommissionedModal()" style="border: 1px solid #cbd5e1; background: #f1f5f9; color: #4a5568;">Cerrar</button>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ============================================================================
    let empleadosData = [];
    let areasData     = []; // ⭐ VARIABLE PARA LAS ÁREAS (CON COMISIONADOS)
    let pozosData     = [];
    let quincenasData = [];
    let mesData       = [];
    let evChart       = null;
   ...
   // ============================================================================
    // FUNCIONES DEL MODAL
    // ============================================================================
    window.openCommissionedModal = function(cell) {
        const rawData = cell.getAttribute('data-commissioned');
        if (!rawData) return;

        const data = JSON.parse(rawData);

        // Calcular totales
        const normalVal = parseFloat(data.normal) || 0;
        const comisionadoVal = parseFloat(data.comisionado) || 0;
        const totalGeneralVal = normalVal + comisionadoVal;

        // Llenar las 3 tarjetas superiores
        document.getElementById('comm-modal-normal').textContent = formatMxn(normalVal);
        document.getElementById('comm-modal-comisionado').textContent = formatMxn(comisionadoVal);
        document.getElementById('comm-modal-total').textContent = formatMxn(totalGeneralVal);

        const tbody = document.getElementById('comm-modal-tbody');
        tbody.innerHTML = '';
        ...
        if (data.detalles && data.detalles.length > 0) {
            ...
            data.detalles.sort((a, b) => new Date(a.fecha) - new Date(b.fecha));

            data.detalles.forEach(d => {
                const montoRegistro = parseFloat(d.monto) || 0;
                sumaTabla += montoRegistro;

                // Formateamos la fecha (ej. 26/03/2026)
                const f = new Date(d.fecha + 'T12:00:00').toLocaleDateString('es-ES', {day:'2-digit', month:'2-digit', year:'numeric'});

                tbody.innerHTML += `
                    <tr>
                        <td class="text-left" style="background: white; border-right: none; border-bottom: 1px solid #e2e8f0;">${f}</td>
                        <td class="text-left" style="background: white; border-right: none; border-bottom: 1px solid #e2e8f0;"><strong>${d.empleado}</strong></td>
                        <td class="text-left" style="background: white; border-right: none; border-bottom: 1px solid #e2e8f0;"><span class="badge-area">${d.area_origen}</span></td>
                        <td class="text-right currency-mxn" style="background: white; border-right: none; border-bottom: 1px solid #e2e8f0;">${formatMxn(montoRegistro)}</td>
                    </tr>
                `;
            });

            // Fila final de totales en la tabla
            tbody.innerHTML += `
                <tr style="background-color: #f8fafc;">
                    <td colspan="3" class="text-right" style="font-weight: 700; color: #4a5568; border-right: none;">TOTAL :</td>
                    <td class="text-right currency-mxn" style="font-weight: 700; color: #0369a1; border-right: none;">${formatMxn(sumaTabla)}</td>
                </tr>
            `;

        } else {
            tbody.innerHTML = `<tr><td colspan="4" class="text-center" style="padding: 15px; color: #64748b; background: white; border-right: none;">No hay detalles disponibles</td></tr>`;
        }

        document.getElementById('commissionedModal').classList.add('show');
    };

    window.closeCommissionedModal = function() {
        document.getElementById('commissionedModal').classList.remove('show');
    };
    // ============================================================================
    // CARGA AJAX
    // ============================================================================
    ...
    window.onclick = function (event) {
        if (!event.target.closest('.multi-select-container') && !event.target.closest('.custom-modal-content')) {
            document.querySelectorAll('.multi-select-options.show')
                    .forEach(el => el.classList.remove('show'));

            // Cierra modal si dan click afuera
            if (event.target.classList.contains('custom-modal-backdrop')) {
                closeCommissionedModal();
            }
        }
    };
    ...
    function renderEmpleados() {
        ...
        empSorted.forEach(emp => {
            // ✅ AHORA LA COINCIDENCIA DE ÁREA SE VERIFICA CORRECTAMENTE CONTRA emp.area
    ...
    // ============================================================================
    // 4. RENDERIZADO — ÁREAS ⭐ ACTUALIZADO PARA MOSTRAR MODAL DE COMISIONADOS
    // ============================================================================
    function renderAreas() {
        ...
        areasSorted.forEach(areaObj => {
            ...
            mesesMostrar.forEach((mes, idx) => {
                ...
                const q1Data = areaObj[k1] || { normal: 0, comisionado: 0, comisionados_detalles: [] };
                const q2Data = areaObj[k2] || { normal: 0, comisionado: 0, comisionados_detalles: [] };
                ...
                const mData = {
                    ...
                    comisionados_detalles: [...(q1Data.comisionados_detalles || []), ...(q2Data.comisionados_detalles || [])]
                };
    ...
    // ⭐ HELPER PARA CREAR LA CELDA QUE DISPARA EL MODAL
    function generateAreaCellHtml(total, data, isTotalMonth = false) {
        ...
        if (data.comisionado > 0) {
            ...
            // Empaquetamos los datos en formato JSON para que los lea el click del JS
            const detailsJson = JSON.stringify({
                normal: data.normal,
                comisionado: data.comisionado,
                detalles: data.comisionados_detalles || []
            }).replace(/"/g, '&quot;');

            dataAttr = `data-commissioned="${detailsJson}" onclick="openCommissionedModal(this)"`;
            icon = `<i class="fas fa-info-circle commissioned-icon"></i> `;
        }

        return `<td class="text-right currency-mxn ${isTotalMonth ? 'total-col' : ''} ${extraClass}" ${dataAttr}>
