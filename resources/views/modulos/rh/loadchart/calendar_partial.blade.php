...
<input type="hidden" id="requires_base_description" value="{{ isset($requiresBaseDescription) && $requiresBaseDescription ? '1' : '0' }}">
...
{{-- NUEVOS CAMPOS DE CONTINUACIÓN Y SUMINISTRO --}}
                <div class="form-group" id="continuation-field" style="display: none; margin-bottom: 15px; background: #eef2f5; padding: 10px; border-radius: 6px;">
                ...
                <div class="form-group" id="contract-number-field" style="display: none;">
                    ...
                    <div class="input-with-icon">
                        <i class="fas fa-file-contract" style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #888;"></i>
                            ...
                            @if(isset($supplyContracts))
                                @foreach($supplyContracts as $contract)
                                    <option value="{{ $contract->number }}">
                                        {{ $contract->number }} @if($contract->short_name) @endif
                                    </option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="error-message" id="contract-number-error">Debes seleccionar el número de contrato</div>
                </div>
                ...
                <div class="form-group" id="travel-service-type-field" style="display: none;">
                    ...
                    <div class="error-message" id="travel-service-type-error">Debes seleccionar el tipo de servicio</div>
                </div>
                {{-- FIN NUEVOS CAMPOS --}}
