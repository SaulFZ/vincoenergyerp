<?php
    ...
    class ApprovalController extends Controller
    {
        ...
        {
            try {
                $request->validate(['employee_id' => 'required|exists:employees,id', 'changes' => 'required|array', 'changes.*.date' => 'required|date_format:Y-m-d', 'changes.*.item_type' => 'required|string', 'changes.*.item_index' => 'nullable|integer', 'changes.*.status' => 'required|string|in:reviewed,approved,rejected', 'changes.*.rejection_reason' => 'nullable|string', 'month' => 'required|integer|min:1|max:12', 'year' => 'required|integer|min:2020|max:2030']);
                ...
                if (! $log) {return response()->json(['success' => false, 'message' => 'Work log not found.'], 404);}
                    ...
                    if ($dailyActivityIndex !== false) {
                        ...
                        if ($activityType === 'VAC' && $itemType === 'activity' && $newStatus === 'approved' && $oldStatus !== 'approved') {
                            if ($balance->vacation_days_available <= 0) {
                                DB::rollback();return response()->json(['success' => false, 'message' => 'No hay días de vacaciones disponibles para aprobar la actividad del día ' . $date . '.'], 422);
                            }
                        }

                        $tempUpdated = false;
                        if ($itemType === 'activity') {
                            $dailyActivity = $this->updateItemStatus($dailyActivity, 'activity_status', $oldStatus, $newStatus, $isReviewer, $isApprover, $tempUpdated);
                            if ($tempUpdated) {$dailyActivity['rejection_reason'] = ($newStatus === 'rejected') ? ($change['rejection_reason'] ?? null) : null;
                                $updated                                                      = true;}
                            ...
                            $item = $this->updateItemStatus($item, 'status', $oldStatus, $newStatus, $isReviewer, $isApprover, $tempUpdated);
                            if ($tempUpdated) {$item['rejection_reason'] = ($newStatus === 'rejected') ? ($change['rejection_reason'] ?? null) : null;
                                $updated                                             = true;}
                        }

                        // --- AJUSTE DE SALDOS (Solo VAC) ---
                        if ($tempUpdated && $oldStatus !== $newStatus) {
                ...
                if ($updated) {$log->daily_activities = $dailyActivities;
                    $log->save();}
                $this->updateLogStatus($log, $newStatus, $userId);
                DB::commit();

                return response()->json([
                    'success'      => true, 'message' => 'Estados actualizados correctamente.', 'updated' => $updated,
                    'new_balances' => $this->getEmployeeBalances($employeeId),
                ]);
            } catch (\Exception $e) {
                DB::rollback();
